...
(function (global, factory) {
    ...
    var ScrollProgress = function(handleUpdate) {
      ...
      // set initial values
      // this._getViewportHeight() = this._getViewportHeight();
      // this._getViewportWidth() = this._getViewportWidth();
  
      this._progress = this._getProgress();
  
      // trigger initial update function
      this._handleUpdate(this._progress.x, this._progress.y, this._progress.direction);
    ...
    ScrollProgress.prototype.oldY = 0;
    ...
    ScrollProgress.prototype._getProgress = function() {
      ...
      var y = typeof window.scrollY === 'undefined'
        ? window.pageYOffset
        : window.scrollY;
      var direction = y > this.oldY
        ? 'down'
        : 'up';
      this.oldY = y;
  
      return {
        x: this._getViewportWidth() === 0
          ? 0
          : x / this._getViewportWidth(),
        y: this._getViewportHeight() === 0
          ? 0
          : y / this._getViewportHeight(),
        direction: direction
      };
    };
